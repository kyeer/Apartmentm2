...
def get_location1():
    ...
    if 'walk' in df.loc[b]['TTC or Car'] or 'ttc' in df.loc[b]['TTC or Car']:
        ...
        try:
            ...
            r = client.distance_matrix(a.strip(), str(results2[
                                                          'If you know the neighbourhood you want to live in please enter it below (Note we will only search apartments in this neighbourhood)'].iloc[
                                                          number]) + ', Ontario', mode='transit')
            if 'NOT_FOUND' in r['rows'][0]['elements'][0]['status']:
                if website == 'craigslist':
                    r = client.distance_matrix(result2[0]['address_components'][-1]['long_name'].replace(' ', ''), str(
                        results2[
                            'If you know the neighbourhood you want to live in please enter it below (Note we will only search apartments in this neighbourhood)'].iloc[
                            number]) + ', Ontario', mode='transit')
                elif website == 'kijiji':
                    r = client.distance_matrix(a.strip()[-6:], str(results2[
                                                                       'If you know the neighbourhood you want to live in please enter it below (Note we will only search apartments in this neighbourhood)'].iloc[
                                                                       number]) + ', Ontario', mode='transit')
        ...
        try:
            ...
            inputElement.clear()
            inputElement.send_keys(str(results2[
                                           'If you know the neighbourhood you want to live in please enter it below (Note we will only search apartments in this neighbourhood)'].iloc[
                                           number]) + ', Ontario')
            ...
            for x in find11:
                if 'hr' not in x.text[:10]:
                    ...
                    if 'car' in df.loc[b]['TTC or Car']:
                        ...
                        break
                    elif 'bike' in df.loc[b]['TTC or Car'] or 'ttc' in df.loc[b]['TTC or Car'] or 'walk' in df.loc[b][
                        'TTC or Car']:
    ...
    if 'walk' in df.loc[b]['TTC or Car'] or 'ttc' in df.loc[b]['TTC or Car']:
        ...
        try:
            ...
            r = client.distance_matrix(a.strip(), str(
                results2['What is the closest intersection to work?'].iloc[number]) + ', Ontario', mode='transit')
            if 'NOT_FOUND' in r['rows'][0]['elements'][0]['status']:
                if website == 'craigslist':
                    r = client.distance_matrix(result2[0]['address_components'][-1]['long_name'].replace(' ', ''), str(
                        results2[
        ...
        try:
            ...
            inputElement.send_keys(
                str(results2['What is the closest intersection to work?'].iloc[number]) + ', Toronto, Ontario')
            inputElement.send_keys(Keys.ENTER)
            time.sleep(2)
            html7 = driver4.page_source
            soup = bs4.BeautifulSoup(html7, 'lxml')
    ...
    if 'walk' in df.loc[b]['TTC or Car'] or 'ttc' in df.loc[b]['TTC or Car']:
        ...
        try:
            ...
            r = client.distance_matrix(a.strip(), str(
                results2['Outside of work, what part of the city are you usually in? '].iloc[number]) + ', Ontario',
                                       mode='transit')
            if 'NOT_FOUND' in r['rows'][0]['elements'][0]['status']:
                if website == 'craigslist':
                    r = client.distance_matrix(result2[0]['address_components'][-1]['long_name'].replace(' ', ''), str(
        ...
        try:
            ...
            inputElement.clear()
            inputElement.send_keys(
                str(results2['Outside of work, what part of the city are you usually in? '].iloc[number]) + ', Ontario')
            inputElement.send_keys(Keys.ENTER)
            time.sleep(2)
            html7 = driver4.page_source
            soup = bs4.BeautifulSoup(html7, 'lxml')
...
display.start()
def create_proxy ():
    proxy = {'host': 'fr.proxymesh.com', 'port': '31280', 'usr': 'kyeer', 'pwd': 'Milou511'}
    this = webdriver.FirefoxProfile()

    this.add_extension('/home/kye_andreopoulos/close_proxy_authentication-1.1-sm+tb+fx.xpi')
    this.set_preference('network.proxy.type', 1)
    this.set_preference('network.proxy.http', proxy['host'])
    this.set_preference('network.proxy.http_port', int(proxy['port']))
    # ... ssl, socks, ftp ...
    this.set_preference('network.proxy.no_proxies_on', 'localhost, 127.0.0.1')
    credentials = '{usr}:{pwd}'.format(**proxy)
    credentials = b64encode(credentials.encode('ascii')).decode('utf-8')
    this.set_preference('extensions.closeproxyauth.authtoken', credentials)
    return this
fp=create_proxy()

driver6 = webdriver.Firefox(fp)
...
time.sleep(5)
driver6.get(
    'http://apartmentrank.com/wp-admin/edit.php?post_status=all&post_type=nf_sub&form_id=3&download_all=all-subs')
link = driver6.find_element_by_link_text('Download All Submissions')
link.click()
time.sleep(10)
for x in os.listdir('/tmp/mozilla_kye_andreopoulos0/'):
    if 'csv.part' in x:
        print x
        break

f=open('/tmp/mozilla_kye_andreopoulos0/apartmentmatchresults.csv','w')
f2=open('/tmp/mozilla_kye_andreopoulos0/'+x,'r')
ths=f2.read()
f.write(ths)
f.close()
f2.close()
results2 = pd.read_csv('/tmp/mozilla_kye_andreopoulos0/apartmentmatchresults.csv',
    error_bad_lines=False)
# results2
results1 = results2[(results2['When were you looking to move in?'] == 'Invalid date')]
results2 = results2[(results2['When were you looking to move in?'] != 'Invalid date') & (
results2['When were you looking to move in?'].str.len() == 10) & (
                    results2['When were you looking to move in?'] != 'short-term')]
...
results2['When were you looking to move in?'] = pd.to_datetime(results2['When were you looking to move in?'],
                                                               format="%d/%m/%Y")
...
results2['Targeted'] = 0
results2['Targeted'][(results2['Datediff'] < 10) & (results2['Datediff'] >= 0) & (
((results2['When were you looking to move in?'] - results2['Date Submitted']) / np.timedelta64(1, 'D')) <= 60)] = 1
results2['Targeted'][(results2['Datediff'] % 7 == 0) & (results2['Datediff'] >= 0) & (
((results2['When were you looking to move in?'] - results2['Date Submitted']) / np.timedelta64(1, 'D')) > 60)] = 1
results2['Targeted'][(results2['Datediff'] == 1) & (
((results2['When were you looking to move in?'] - results2['Date Submitted']) / np.timedelta64(1, 'D')) > 60)] = 1
results2['Targeted'][(results2['Datediff'] == 2) & (
((results2['When were you looking to move in?'] - results2['Date Submitted']) / np.timedelta64(1, 'D')) > 60)] = 1
...
for number in range(1,3):
    ...
    fp2.set_preference("javascript.enabled", False)
    fp3.set_preference("javascript.enabled", False)
    fp4.set_preference("javascript.enabled", False)
    fp5.set_preference("javascript.enabled", False)
    fp6.set_preference("javascript.enabled", False)
    fp7.set_preference("javascript.enabled", False)
    fp8.set_preference("javascript.enabled", False)
    fp9.set_preference("javascript.enabled", False)
    fp10.set_preference("javascript.enabled", False)
    fp11.set_preference("javascript.enabled", False)

    h = []
    index = range(0, 10000)
    df = pd.DataFrame(index=index,
                      columns=('Website', 'ApartmentType', 'WorkAddress', 'HangoutAddress', 'ChosenAddress',
                               ...
                               'Min Price', 'Max Price', 'Apartment Price', 'Basement or Above Ground', 'Pet Friendly',
                               'PostalCode', 'WentWrong'))

    pd106 = pd.DataFrame(columns=(
    'Apartment Location', 'Pharmacy', 'Restaurant', 'Liquor Store', 'Grocery', 'Park', 'Gym', 'CoffeeShop'))

    print 'PERSON ' + str(number)
    b = 0
    i = 0
    websitelist = [ 'kijiji', 'craigslist']
    for website in websitelist:
        ...
        if website == 'kijiji':
            ...
            if 'bachelor' in results2['How Many Bedrooms Were You Looking For?'].iloc[number]:
                apartmenttype.append(
                    'http://www.kijiji.ca/b-bachelor-studio-apartments-condos/city-of-toronto/c211l1700273')
            if 'one' in results2['How Many Bedrooms Were You Looking For?'].iloc[number]:
                apartmenttype.append(
                    'http://www.kijiji.ca/b-1-bedroom-den-apartments-condos/city-of-toronto/c213l1700273')
            ...
            for apartmenttype2 in apartmenttype:
                ...
                for z in kijijilist:
                    ...
                    for x in find:
                        ...
                        df.loc[b]['Closer to Home or Work'] = \
                        results2['Would you prefer to live closer to work or where you spend your free time?'].iloc[
                            number]
                        ...
                        try:
                            ...
                            for x in find2:
                                ...
                                if 'Address' in x.text.strip():
                                    ...
                                    df.loc[b]['WorkAddress'] = str(
                                        results2['What is the closest intersection to work?'].iloc[
                                            number]) + ', Toronto, Ontario, Canada'
                                    df.loc[b]['HangoutAddress'] = str(
                                        results2['Outside of work, what part of the city are you usually in? '].iloc[
                                            number]) + ', Ontario, Canada'
                                    try:
                                        df.loc[b]['ChosenAddress'] = str(results2[
                                                                             'If you know the neighbourhood you want to live in please enter it below (Note we will only search apartments in this neighbourhood)'].iloc[
                                                                             number]) + ', Ontario'
                                    except:
                        ...
                        if apartmentprice <= int(df.loc[b]['Max Price']) and apartmentprice >= int(
                                df.loc[b]['Min Price']):
                            pass
                        else:
                            b = b + 1
    ...
    try:
        ...
        pass
    try:
        df['Total Distance'][(df['Closer to Home or Work'] == 'work') & (df['Hangout Time'] < df['Work Time'])] = df[
                                                                                                                      'Work Time'] + \
                                                                                                                  df[
                                                                                                                      'Work Time'] + 5
        df['Total Distance'][
            (df['Closer to Home or Work'] == 'outside-of-work') & (df['Hangout Time'] > df['Work Time'])] = df[
                                                                                                                'Hangout Time'] + \
                                                                                                            df[
                                                                                                                'Hangout Time'] + 5
    ...
    for x in range(0, len(df.index)):
        ...
        print numberx
        driver13 = webdriver.Firefox(fp5)
        ...
        driver14 = webdriver.Firefox(fp6)
        ...
        driver15 = webdriver.Firefox(fp7)
        ...
        driver16 = webdriver.Firefox(fp8)
        ...
        driver17 = webdriver.Firefox(fp9)
        ...
        driver18 = webdriver.Firefox(fp10)
        # location=a.strip().replace(" ", "+")
        ...
        driver19 = webdriver.Firefox(fp11)
        # location=a.strip().replace(" ", "+")
        ...
        pd106 = pd106.append(pd105[['Apartment Location', 'Pharmacy', 'Restaurant', 'Liquor Store', 'Grocery', 'Park',
                                    'Gym', 'CoffeeShop']])
    ...
    # GET POSTALCODE
    df10 = pd.read_excel(df10=pd.read_excel('/home/Kyeer/Environics.xlsx'))
    ...
    if 'cook' in results3['Do you prefer to cook at home or eat out?'][0]:
        ...
        dffinal2['Do you prefer to cook at home or eat out?'] = dffinal2[
                                                                    'Restaurant Within 10 Mins'] + ' within 10 mins'
    ...
    dffinal2.rename(columns={'median_household_incomeconstant_year_2005_dollars': 'Average Income of Neighbourhood'},
                    inplace=True)
    dffinal2['Chosen Time'] = dffinal2['Chosen Time'].fillna(15)
    if 'walk' not in dffinal2['TTC or Car'][0]:
        dffinal2['What is the closest intersection to work?'] = dffinal2['Work Time'].astype(int).astype(
            str) + ' mins by ' + dffinal2['TTC or Car']
        dffinal2['Outside of work, what part of the city are you usually in? '] = dffinal2['Hangout Time'].astype(
            int).astype(str) + ' mins by ' + dffinal2['TTC or Car']

        dffinal2[
            'If you know the neighbourhood you want to live in please enter it below (Note we will only search apartments in this neighbourhood)'] = \
        dffinal2['Chosen Time'].astype(int).astype(str) + ' mins by ' + dffinal2['TTC or Car']
    else:

        dffinal2['What is the closest intersection to work?'] = dffinal2['Work Time'].astype(int).astype(
            str) + ' mins by ' + 'TTC'
        dffinal2['Outside of work, what part of the city are you usually in? '] = dffinal2['Hangout Time'].astype(
            int).astype(str) + ' mins by ' + 'TTC'

        dffinal2[
            'If you know the neighbourhood you want to live in please enter it below (Note we will only search apartments in this neighbourhood)'] = \
        dffinal2['Chosen Time'].astype(int).astype(str) + ' mins by ' + 'TTC'
    ...
    if 'wine' in results3['At dinner you usually drink...'][0] and (
                'town' in results2['After work you usually...'][number] or 'Park' in
            results2['After work you usually...'][number] or 'Gym' in results2['After work you usually...'][number]):
        dffinal3 = finalresult[['Basement or Above Ground', 'Photos', 'ApartmentAddress',
                                'Apartment Price Range (including utilities, parking)',
                                'How Many Bedrooms Were You Looking For?', 'Bed Bug Reports in Last 2 Years',
                                'What is the closest intersection to work?',
                                'Outside of work, what part of the city are you usually in? ',
                                'If you know the neighbourhood you want to live in please enter it below (Note we will only search apartments in this neighbourhood)',
                                'Do you have any pets?', 'Do you prefer space or sunlight?',
                                'Are you looking for short-term or long-term?',
                                'Do you prefer to cook at home or eat out?', 'After work you usually...',
                                'At dinner you usually drink...', 'Neighbourhood', 'For whom?',
                                'What age are you, in spirit?', 'Safety of Neighbourhood',
                                'Average Income of Neighbourhood', 'Gym Within 10 Mins', 'Pharmacy Within 10 Mins',
                                'Restaurant Within 10 Mins', 'Grocery Within 10 Mins', 'Park Within 10 Mins',
                                'Coffee Shop Within 10 Mins', 'Walkscore', 'When were you looking to move in?',
                                'Website']]
    elif 'wine' not in results3['At dinner you usually drink...'][0] and (
                'town' in results2['After work you usually...'][number] or 'Park' in
            results2['After work you usually...'][number] or 'Gym' in results2['After work you usually...'][number]):
        dffinal3 = finalresult[['Basement or Above Ground', 'Photos', 'ApartmentAddress',
                                'Apartment Price Range (including utilities, parking)',
                                ...
                                'Are you looking for short-term or long-term?',
                                'Do you prefer to cook at home or eat out?', 'After work you usually...',
                                'Neighbourhood', 'For whom?', 'What age are you, in spirit?', 'Safety of Neighbourhood',
                                'Average Income of Neighbourhood', 'Gym Within 10 Mins', 'Pharmacy Within 10 Mins',
                                'Restaurant Within 10 Mins', 'Grocery Within 10 Mins', 'Park Within 10 Mins',
                                'Coffee Shop Within 10 Mins', 'Walkscore', 'When were you looking to move in?',
                                'Website']]
    elif 'wine' in results3['At dinner you usually drink...'][0] and 'town' not in \
            results2['After work you usually...'][number] and 'Park' not in results2['After work you usually...'][
        number] and 'Gym' not in results2['After work you usually...'][number]:
        dffinal3 = finalresult[['Basement or Above Ground', 'Photos', 'ApartmentAddress',
                                'Apartment Price Range (including utilities, parking)',
                                'How Many Bedrooms Were You Looking For?', 'Bed Bug Reports in Last 2 Years',
                                'What is the closest intersection to work?',
                                ...
                                'Do you have any pets?', 'Do you prefer space or sunlight?',
                                'Are you looking for short-term or long-term?',
                                'Do you prefer to cook at home or eat out?', 'At dinner you usually drink...',
                                'Neighbourhood', 'For whom?', 'What age are you, in spirit?', 'Safety of Neighbourhood',
                                'Average Income of Neighbourhood', 'Gym Within 10 Mins', 'Pharmacy Within 10 Mins',
                                'Restaurant Within 10 Mins', 'Grocery Within 10 Mins', 'Park Within 10 Mins',
                                'Coffee Shop Within 10 Mins', 'Walkscore', 'When were you looking to move in?',
                                'Website']]
    elif 'wine' not in results3['At dinner you usually drink...'][0] and 'town' not in \
            results2['After work you usually...'][number] and 'Park' not in results2['After work you usually...'][
        number] and 'Gym' not in results2['After work you usually...'][number]:
        dffinal3 = finalresult[['Basement or Above Ground', 'Photos', 'ApartmentAddress',
                                'Apartment Price Range (including utilities, parking)',
                                ...
                                'Do you have any pets?', 'Do you prefer space or sunlight?',
                                'Are you looking for short-term or long-term?',
                                'Do you prefer to cook at home or eat out?', 'Neighbourhood', 'For whom?',
                                'What age are you, in spirit?', 'Safety of Neighbourhood',
                                'Average Income of Neighbourhood', 'Gym Within 10 Mins', 'Pharmacy Within 10 Mins',
                                'Restaurant Within 10 Mins', 'Grocery Within 10 Mins', 'Park Within 10 Mins',
                                'Coffee Shop Within 10 Mins', 'Walkscore', 'When were you looking to move in?',
    ...
    results3[
        'If you know the neighbourhood you want to live in please enter it below (Note we will only search apartments in this neighbourhood)'] = \
    results3[
        'If you know the neighbourhood you want to live in please enter it below (Note we will only search apartments in this neighbourhood)'].fillna(
        '0')
    abba = results3[
        'If you know the neighbourhood you want to live in please enter it below (Note we will only search apartments in this neighbourhood)'].iloc[
        [0]]
    ...
    for images in range(0, len(dffinal4.columns)):
        ...
        print imagecount
        import pip
        pip.main(['install', 'openyxl'])
        from openpyxl import Workbook
        from openpyxl import load_workbook

        wb = load_workbook('/home/Kyeer/Master Tracker Test.xlsx')
        ...
        websitecheck = 'https://maps.googleapis.com/maps/api/streetview/metadata?size=600x600&location=' + \
                       dffinal4[column][
                           'ApartmentAddress'] + '&fov=90&heading=235&pitch=10&key=AIzaSyCouL7oS-mK8sfx_TDdQQ8GfoU7WDeNRuQ'
            ...
            if s.isdigit():
                ...
                if 'ZERO_RESULTS' in site.json():
                    ...
                    try:
                        ...
                        if locationlist != '':
                            locationlist = locationlist + '&markers=label:' + str(secondnumber) + '%7C' + \
                                           dffinal4[column]['ApartmentAddress'].replace('&', ' ').replace(
                                               'google map, Canada', ' ')
                            secondnumber = secondnumber + 1
                            goodmatch.append(column)
                        else:
                            locationlist = 'label:' + str(secondnumber) + '%7C' + dffinal4[column][
                                'ApartmentAddress'].replace('&', ' ').replace('google map, Canada', ' ')
                            secondnumber = secondnumber + 1
                            goodmatch.append(column)
                        break
                    except:
                ...
                if imagecount == 0:
                    ...
                    print image1
                    if locationlist != '':
                        locationlist = locationlist + '&markers=label:' + str(secondnumber) + '%7C' + dffinal4[column][
                            'ApartmentAddress'].replace('&', ' ').replace('google map, Canada', ' ')
                        secondnumber = secondnumber + 1
                        goodmatch.append(column)
                        break
        ...
        try:
            ...
            if locationlist != '':
                locationlist = locationlist + '&markers=label:' + str(secondnumber) + '%7C' + dffinal4[column][
                    'ApartmentAddress'].replace('&', ' ').replace('googlemap', ' ')
                secondnumber = secondnumber + 1
                goodmatch.append(column)
            else:
                locationlist = 'label:' + str(secondnumber) + '%7C' + dffinal4[column]['ApartmentAddress'].replace('&',
                                                                                                                   ' ').replace(
                    'googlemap', ' ')
                secondnumber = secondnumber + 1
                goodmatch.append(column)
    ...
    try:
        hyperlink1 = '''<a href=''' + dffinal4[goodmatch[0]][
            'Website'] + '''>Check out the comparison below, then click here to see this apartment.''' + '''</a>'''
    except:
        pass
    try:
        hyperlink2 = '''<a href=''' + dffinal4[goodmatch[1]][
            'Website'] + '''>Check out the comparison below, then click here to see this apartment.''' + '''</a>'''
    except:
        pass
    try:
        hyperlink3 = '''<a href=''' + dffinal4[goodmatch[2]][
            'Website'] + '''>Check out the comparison below, then click here to see this apartment.''' + '''</a>'''
    except:
        pass
    ...
    try:
        if img1 == '':
            match1 = '''<b>''' + dffinal4[goodmatch[0]][
                'Apartment Price Range (including utilities, parking)'] + '''</b><br>Distance from work: <b>''' + \
                     dffinal4[goodmatch[0]][
                         'What is the closest intersection to work?'] + '''<br></b>Distance from Hangout: <b>''' + \
                     dffinal4[goodmatch[0]][
                         'Outside of work, what part of the city are you usually in? '] + '''</b><br>Apartment Type: <b>''' + \
                     dffinal4[goodmatch[0]][
                         'How Many Bedrooms Were You Looking For?'] + '''<br></b>Basement Or Above-Ground: <b>''' + \
                     dffinal4[goodmatch[0]]['Basement or Above Ground'] + '''</b><br>'''
            try:
                match1 = match1 + '''Allows Pets:<b>''' + dffinal4[goodmatch[0]][
                    'Do you have any pets?'] + '''</b><br>''' + str(hyperlink1) + '''<br><br>''' + image1
            except:
            ...
            match1 = '''<b>''' + dffinal4[goodmatch[0]][
                'Apartment Price Range (including utilities, parking)'] + '''</b><br>Distance from work: <b>''' + \
                     dffinal4[goodmatch[0]][
                         'What is the closest intersection to work?'] + '''</b><br>Distance from Hangout: <b>''' + \
                     dffinal4[goodmatch[0]][
                         'Outside of work, what part of the city are you usually in? '] + '''</b><br>Apartment Type: <b>''' + \
                     dffinal4[goodmatch[0]][
                         'How Many Bedrooms Were You Looking For?'] + '''</b><br>Basement Or Above-Ground: <b>''' + \
                     dffinal4[goodmatch[0]]['Basement or Above Ground'] + '''</b><br>'''
            try:
                match1 = match1 + '''Allows Pets: <b>''' + dffinal4[goodmatch[0]][
                    'Do you have any pets?'] + '''</b><br>''' + str(
                    hyperlink1) + '<br>' + '''<br>''' + '''<img src="cid:image10"style="width:500px;height:300px;">'''
            except:
                match1 = match1 + str(
                    hyperlink1) + '<br><br>' + '''<img src="cid:image10"style="width:500px;height:300px;">'''

        if img2 == '':
            match2 = '''<b>''' + dffinal4[goodmatch[1]][
                'Apartment Price Range (including utilities, parking)'] + '''</b><br>Distance from work: <b>''' + \
                     dffinal4[goodmatch[1]][
                         'What is the closest intersection to work?'] + '''</b><br>Distance from Hangout: <b>''' + \
                     dffinal4[goodmatch[1]][
                         'Outside of work, what part of the city are you usually in? '] + '''</b><br>Apartment Type: <b>''' + \
                     dffinal4[goodmatch[1]][
                         'How Many Bedrooms Were You Looking For?'] + '''</b><br>Basement Or Above-Ground: <b>''' + \
                     dffinal4[goodmatch[1]]['Basement or Above Ground'] + '''</b><br>'''
            try:
                match2 = match2 + '''Allows Pets: <b>''' + dffinal4[goodmatch[1]][
                    'Do you have any pets?'] + '</b><br>' + str(hyperlink2) + '<br>' + '''<br>''' + nextimage
            except:
            ...
            try:
                match2 = match2 + '''Allows Pets: <b>''' + dffinal4[goodmatch[1]][
                    'Do you have any pets?'] + '''</b><br>''' + str(
                    hyperlink2) + '<br>' + '''<br>''' + '''<img src="cid:image11"style="width:500px;height:300px;">'''
            except:
                match2 = match2 + str(
                    hyperlink2) + '<br>' + '''<br>''' + '''<img src="cid:image11"style="width:500px;height:300px;">'''

        if img3 == '':
            match3 = '''<b>''' + dffinal4[goodmatch[2]][
                'Apartment Price Range (including utilities, parking)'] + '''</b><br>Distance from work: <b>''' + \
                     dffinal4[goodmatch[2]][
                         'What is the closest intersection to work?'] + '''</b><br>Distance from Hangout: <b>''' + \
                     dffinal4[goodmatch[2]][
                         'Outside of work, what part of the city are you usually in? '] + '''</b><br>Apartment Type: <b>''' + \
                     dffinal4[goodmatch[2]][
                         'How Many Bedrooms Were You Looking For?'] + '''</b><br>Basement Or Above-Ground: <b>''' + \
                     dffinal4[goodmatch[2]]['Basement or Above Ground'] + '''</b><br>'''
            try:
                match3 = match3 + '''Allows Pets: <b>''' + dffinal4[goodmatch[2]][
                    'Do you have any pets?'] + '''</b><br>''' + str(hyperlink3) + '<br>' + '''<br>''' + nextnextimage
            except:
            ...
            try:
                match3 = match3 + '''Allows Pets: <b>''' + dffinal4[goodmatch[2]][
                    'Do you have any pets?'] + '''</b><br>''' + str(
                    hyperlink3) + '<br>' + '''<br>''' + '''<img src="cid:image12"style="width:500px;height:300px;">'''
            except:
                match3 = match3 + str(
                    hyperlink3) + '<br>' + '''<br>''' + '''<img src="cid:image12"style="width:500px;height:300px;">'''
    except:
    ...
    testlist = ['Gym Within 10 Mins', 'Pharmacy Within 10 Mins', 'Restaurant Within 10 Mins', 'Grocery Within 10 Mins',
                'Park Within 10 Mins', 'Coffee Shop Within 10 Mins']
    ...
    if match2 == '':
        ...
        stringlist = (
        str(dffinal4[goodmatch[0]]['ApartmentAddress']) + '''</b><br>''' + str(match1) + '<b><br><br><br><br>' +
    ...
    data = (HEADER + image2 + '''<br />''' + '''<br />''' + '''<br />'''
            ...
            '<b> Mostly Singles or Families: </b> ' + '''<br><br>''' + dfneighbour.to_html(classes='df').encode(
        'utf8') + '''<br>'''
    ...
    for x in range(0, 100):
        try:
            ...
            result, data = conn.fetch(latest_email_id,
